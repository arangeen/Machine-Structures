...
main:
    ...
    blt $s0,$t0, ELSE  # if( a < 10 )
    addi $s0, $s0, -1   # else a— 
    
   
	
    ELSE: addi $s0, $s0, 1  # else a++
    
    
     add $s3, $s0, $s2  # d = a + c
     add $s2, $s0, $s3  # c = a + d

	blt $s1,$t0, ELSE2  # if( b < 10 )
    	addi $s1, $s1, -1   # else: b—
	addi $s2,$s2, 1   # c++  
		...
		addi $s2, $s2, -1  # c—
		 ...
		 add $s3, $s0, $s1     # d = a + b
   		 ...
   		 ELSE3: bgt $s1,$s2,in_if     #else if ( b > c || c <a)
    			blt $s2, $s0, exit	
...
exit:
    ...
    addi $v0,$0, 10    # syscall to end program 
    syscall